    ...
    <div className="min-h-screen bg-[#0a0e1a] text-white">
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 space-y-6">
        ...
        <Section title="Dashboard Widgets">
          <p className="text-xs text-white/40 mb-3">Toggle widgets on/off here. Use the pencil icon on the dashboard to drag, drop, and resize widgets.</p>
          <div className="space-y-2">
            {(config.layout?.widgets || DEFAULT_GRID_WIDGETS).map((wc, idx) => {
              const def = WIDGET_DEFINITIONS.find((d) => d.id === wc.i);
              if (!def) return null;
              ...
              return (
                <div key={wc.i} className={`flex items-center gap-3 px-3 py-2 rounded-lg ${wc.enabled ? "bg-white/5" : "bg-white/[0.02] opacity-50"}`}>
          ...
          <button
            onClick={() => updateConfig("layout.widgets", DEFAULT_GRID_WIDGETS)}
